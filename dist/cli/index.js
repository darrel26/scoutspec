...
function injectSkills(options) {
  ...
  for (const agent of options.agents) {
    for (const skillName of skillNames) {
      const skillSrc = path3.join(templatesDir, "skills", skillName);
      let targetSkillDir = "";
      if (agent === "claude") {
        targetSkillDir = path3.join(cwd, ".claude", "skills", skillName);
      } else if (agent === "opencode") {
        targetSkillDir = path3.join(cwd, ".opencode", "skills", skillName);
      }
      if (targetSkillDir) {
        const files = copyDirectoryRecursive(skillSrc, targetSkillDir, options.overwrite ?? true);
        results[agent].push(...files.map((f) => path3.relative(cwd, f)));
      }
    }
  }
